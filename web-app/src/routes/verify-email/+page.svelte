<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			const response = await fetch(`/api/auth/verify-email?email=${encodeURIComponent(email)}&code=${encodeURIComponent(code)}`);
		...
		{:else if status === 'success'}
			<div class="bg-gradient-to-br from-green-900/30 to-black rounded-xl border border-green-500/50 p-8">
				...
				<p class="text-sm text-gray-400 mb-6">
					Your account is now fully activated. You can start trading!
				</p>
				<a
					href="/login"
